...
## Local Vocabulary Inbox

`src/vocab-inbox.js` manages a **device-local capture queue** for construction terms the owner notices during the day.

Rules:

- The Inbox is temporary browser/localStorage state, not repository data and not a vocabulary source of truth.
...
- Keep Inbox persistence resilient to unavailable/corrupt localStorage and bound its size.
- The PWA validator must recursively include runtime modules such as `vocab-inbox.js` in the offline precache graph.
