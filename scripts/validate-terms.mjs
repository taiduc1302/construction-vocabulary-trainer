...
for(const [index,t] of terms.entries()){
  ...
  if(seen.has(t.id))errors.push(`${t.id}: duplicate id`);seen.add(t.id);
...
console.log(`OK: ${terms.length} terms across ${new Set(terms.map(t=>t.category)).size} used categories.`);
